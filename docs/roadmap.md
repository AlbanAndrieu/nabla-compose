...
15. [ ] Normalize Sample database ownership: TrueNAS staging depends on shared PostgreSQL at `172.17.0.24:5432`; create a dedicated least-privilege Sample database/role, keep local `POSTGRES_*` for that service, move the historical Supabase pooler identity to explicit `SUPABASE_*` variables in `fastapi-sample`, and remove the duplicate/overloaded `POSTGRES_USER` contract.
