...
3. **AutoXpose -> Cloudflare DNS** for Docker services explicitly assigned to
   AutoXpose DNS ownership, such as `sample.albandrieu.com`.
4. Legacy DDNS / Traefik Cloudflare companion services that remain in the
...
For a direct public hostname such as `sample.albandrieu.com`, the DNS record
must resolve to the pfSense WAN address and HAProxy must have a host ACL/backend
path that forwards that hostname to Traefik while preserving the HTTP Host.
