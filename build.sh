...
if [ "$HOSTNAME" = albandrieu ]; then
  ...
  docker compose --env-file .env --env-file .env.secrets -f docker-compose-${HOSTNAME}.yml up postgres -d
  docker compose --env-file .env --env-file .env.secrets -f docker-compose-${HOSTNAME}.yml up --no-build redis-server -d
  ...
  # docker compose --env-file .env --env-file .env.secrets -f docker-compose-${HOSTNAME}.yml up --pull=never clickhouse -d
  # # docker compose --env-file .env --env-file .env.secrets -f docker-compose-${HOSTNAME}.yml up -d --force-recreate clickhouse
  # docker compose --env-file .env --env-file .env.secrets -f docker-compose-${HOSTNAME}.yml up --pull=never vroom-cleanup -d
