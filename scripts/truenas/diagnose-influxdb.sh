...
require_root "run with sudo"
require_commands curl docker jq midclt
...
probe() {
  ...
  code="$(
    curl -sS \
      --connect-timeout 3 \
      --max-time 8 \
      -o /tmp/nabla-influx-health.json \
      -w '%{http_code}' \
...
if docker inspect "${CONTAINER}" >/dev/null 2>&1; then
  ...
  if docker exec "${CONTAINER}" \
    curl -fsS --connect-timeout 3 --max-time 8 "${INTERNAL_URL}/health"; then
