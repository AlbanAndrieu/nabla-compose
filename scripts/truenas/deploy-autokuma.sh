...
[[ -s "${SECRET_FILE}" ]] ||
  fail "AutoKuma runtime secret file is empty: ${SECRET_FILE}"
...
docker compose   -f apps/autokuma/compose.yml   config   --quiet   --no-interpolate   --no-env-resolution
...
compose_path="${ROOT}/apps/autokuma/compose.yml"
