...
for ((attempt = 1; attempt <= ATTEMPTS; attempt++)); do
  ...
  if jq -e 'type == "object" and (.trace_id | type == "string")' >/dev/null 2>&1 <<<"${ERROR_JSON}"; then
    break
  fi
  ERROR_JSON=""
  sleep "${DELAY_SECONDS}"
...
GROUP_ID="$(jq -r '.group_id // empty' <<<"${ERROR_JSON}")"
STORED_TRACE_ID="$(jq -r '.trace_id // empty' <<<"${ERROR_JSON}")"
TRANSACTION_NAME="$(jq -r '.transaction_name // empty' <<<"${ERROR_JSON}")"
[[ "${STORED_TRACE_ID}" == "${TRACE_UUID}" ]] ||
  fail "Sentry error trace mismatch: expected=${TRACE_UUID} stored=${STORED_TRACE_ID:-<empty>}"
printf '✅ Sentry error persisted and correlated: issue=%s transaction=%s\n' \
  "${GROUP_ID:-<none>}" "${TRANSACTION_NAME:-<none>}"
...
for ((attempt = 1; attempt <= ATTEMPTS; attempt++)); do
  ...
  if ((EAP_SPANS + TRANSACTIONS > 0)); then
    break
  fi
  sleep "${DELAY_SECONDS}"
done
...
if ((EAP_SPANS + TRANSACTIONS < 1)); then
  fail "Sentry error is correlated to trace ${TRACE_ID}, but no transaction/span was persisted after $((ATTEMPTS * DELAY_SECONDS))s (eap_spans=${EAP_SPANS}, transactions=${TRANSACTIONS})"
fi
