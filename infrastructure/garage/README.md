...
1. Create a `opentofu-state` bucket in Garage (used as the remote state backend for all infrastructure units)
...
- **1Password.Vaultwarden items** — credentials for the access key are automatically stored in the `Automation` vault
