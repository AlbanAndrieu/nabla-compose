...
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_PATH}/compose.arr.yml"
...
services:
  ...
  radarr:
    ...
    environment:
      ...
      - TZ=${CONFIG_TZ}
    volumes:
      - /mnt/${CONFIG_POOL}/radarr/${CONFIG_DIR}:/config
      - /mnt/${MEDIA_POOL}/media:/media
  ...
  sonarr:
    ...
    environment:
      ...
      - TZ=${CONFIG_TZ}
    volumes:
      - /mnt/${CONFIG_POOL}/sonarr/${CONFIG_DIR}:/config
      - /mnt/${MEDIA_POOL}/media:/media
  ...
  seerr:
    ...
    restart: unless-stopped
    volumes:
      - /mnt/${CONFIG_POOL}/seerr/${CONFIG_DIR}:/app/config
  ...
  bazarr:
    ...
    environment:
      ...
      - TZ=${CONFIG_TZ}
    volumes:
      - /mnt/${CONFIG_POOL}/bazarr/${CONFIG_DIR}:/config
      - /mnt/${MEDIA_POOL}/media:/media
...
read -r -p "Would you like to launch the Docker containers now? (yes/no): " LAUNCH_CONTAINERS
...
if [[ "${LAUNCH_CONTAINERS}" =~ ^[Yy]es$ ]]; then
  ...
  if   docker compose --env-file .env --env-file .env.secrets -f docker-compose-"${HOSTNAME}".yml -f "${DOCKER_COMPOSE_FILE}" up -d; then
    ...
    RADARR_CONFIG_FILE="/mnt/${CONFIG_POOL}/radarr/${CONFIG_DIR}/config.xml"
    SONARR_CONFIG_FILE="/mnt/${CONFIG_POOL}/sonarr/${CONFIG_DIR}/config.xml"
