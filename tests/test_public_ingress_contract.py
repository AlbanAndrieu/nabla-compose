...
class PublicIngressContractTests(unittest.TestCase):
    ...
    def test_truenas_observer_preflight_is_read_only_and_allowlist_aware(self) -> None:
        ...
        self.assertIn('adapter._call("auth.me")', script)
        self.assertIn("TRUENAS_OBSERVER_EXPECTED_USERNAME", script)
        self.assertIn("fastapi_observer", script)
        self.assertIn("FULL_ADMIN", script)
        self.assertIn('"_WRITE" in role', script)
        self.assertIn("(list, tuple, set, frozenset)", script)
        self.assertIn('if isinstance(raw_roles, dict)', script)
        self.assertIn('"APPS_READ" not in roles', script)
        self.assertIn('scope = "broad_readonly" if "READONLY_ADMIN" in roles else "apps_read"', script)
        ...
        self.assertIn("/v2/version", script)
        self.assertIn("local FastAPI version", script)
        self.assertIn("cloud FastAPI version", script)
        self.assertIn("local catalogRevision", script)
        self.assertIn("cloud catalogRevision", script)
        self.assertIn("catalog-schema compatibility/cache state", script)
